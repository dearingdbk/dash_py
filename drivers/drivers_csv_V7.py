...
def get_start_date():
  ...
  try:
    ...
    if date_format is not None:
      get_start_date() # continuous loop until proper date entered.
    else:
      print ("Entry of starting date of week cancelled. Exiting")
      conn.close()
      exit()
  return
...
def get_append_date():
  ...
  ROOT.withdraw()
  global FOLDER_DATE
  date_format = simpledialog.askstring(title="File Name Date",
                                      prompt="Enter the date you want appended to files in YYYY-MM-DD format")
  try:
    FOLDER_DATE = datetime.strptime(date_format, "%Y-%m-%d")
  except:
    if date_format is not None:
      get_append_date() # continuous loop until proper date entered.
    else:
  ...
  def calendar_gui():
    ...
    ttk.Button(root, text='Set Start Date', command=calendar_view).pack(padx=20, pady=20)
    ttk.Button(root, text='Set Folder Date', command=calendar_view).pack(padx=20, pady=20)
    ttk.Button(root, text='Close', command=close_windows).pack(padx=20, pady=20)
    #calendar_view()
    root.mainloop()
#######################################################################################
# driver_pay(name):                                                                   #
